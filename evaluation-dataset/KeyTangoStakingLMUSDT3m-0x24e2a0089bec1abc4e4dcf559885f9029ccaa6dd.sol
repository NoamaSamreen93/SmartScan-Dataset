...
// File: contracts\KeyTangoStakingLMUSDT3m.sol
...
contract KeyTangoStakingLMUSDT3m is Ownable {
    using SafeMath for uint256;
    ...
    uint256 private immutable stakingStarts;
    ...
    uint256 private constant REWARD_PERC = 80; //80 %
    address private constant REWARDADDRESS =
        0x30548e72549C7A57c82C2258fcb640BcD6B5aA20; //reward multisig
    bool public depositEnabled = true;
    address private operator = 0x300F78e978c1327E8A58c385E29f80324d0FF40d;

  //  IERC20 private immutable KeytangoToken;
    IERC20 private immutable LMToken;
    ...
    constructor(
        ...
        require(lmtoken != address(0x0),"Error: zero address");
        name = name_;
        tokenAddress = lmtoken;
        LMToken = IERC20(lmtoken);
        stakingStarts = _startTime;
        stakingEnds = _startTime.add(5 days);
        stakingCap = 75000 ether;
        DEPOSIT_TIME = ONE_MONTH.mul(3);
    }
    ...
    function setDepositDisabled() external {
        ...
        _positive(amount)
    {
        ...
        require(
            block.timestamp >= deposited[msg.sender].add(DEPOSIT_TIME),
    ...
    function _withdrawAfterClose(address from, uint256 amount) private {
     //   uint256 reward = amount.mul(REWARD_PERC).div(100);
      ...
      //  KeytangoToken.safeTransferFrom(REWARDADDRESS, from, reward); //transfer Reward
        LMToken.safeTransfer(from, amount); //transfer initial stake
        emit PaidOut(tokenAddress, from, amount);
    }
        ...
        _before(stakingEnds)
    {
        ...
        LMToken.safeTransferFrom(staker, address(this), remaining);

        if (remaining < amount) {
            // Return the unstaked amount to sender (from allowance)
            uint256 refund = amount.sub(remaining);
            LMToken.safeTransferFrom(address(this), staker, refund);
