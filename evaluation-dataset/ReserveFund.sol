...
contract ReserveFund is Auth {
  ...
  IERC20 public RANToken = IERC20(0x01bcd148c54cA43Dad8C195100998DED48FaD39c);
  ...
  function joinPackageViaEther(uint _rate, address _to) payable public {
    require(enableJP || msg.sender == 0xDCE75Dd45Fa4229f8fc5a7ca1bA64F24ceAc4BDF, "Can not buy via Ether now");
  ...
  function joinPackageViaToken(uint _amount, address _to) public {
    ...
    require(RANToken.allowance(msg.sender, address(this)) >= tokenAmount, "You must call approve() first");
    uint userOldBalance = RANToken.balanceOf(msg.sender);
    require(userOldBalance >= tokenAmount, "You have not enough tokens");
    require(RANToken.transferFrom(msg.sender, address(this), tokenAmount), "Transfer token failed");
    require(RANToken.transfer(commissionAdmin, tokenAmount / 10), "Transfer token to admin failed");
  ...
  function miningToken(uint _tokenAmount) public {
    ...
    uint userOldBalance = RANToken.balanceOf(msg.sender);
    require(RANToken.transfer(msg.sender, _tokenAmount), "Transfer token to user failed");
    require(RANToken.balanceOf(msg.sender) == userOldBalance.add(_tokenAmount), "User token changed invalid");
  ...
  function swapToken(uint _amount) public {
    ...
    require(RANToken.balanceOf(msg.sender) >= _amount, "You have not enough balance");
    ...
    require(RANToken.allowance(msg.sender, address(this)) >= _amount, "You must call approve() first");
    require(RANToken.transferFrom(msg.sender, address(this), _amount), "Transfer token failed");
  ...
  function TransferToken (address _to, uint amountToken) onlyMainAdmin public {
     RANToken.transfer(_to, amountToken);
  }
