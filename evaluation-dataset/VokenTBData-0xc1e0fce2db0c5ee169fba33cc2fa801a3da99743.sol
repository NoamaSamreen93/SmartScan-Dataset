...
// File: contracts/strategies/StabilizeStrategyETHArbV2.sol
...
// Added slippage protection and max batch sizes
...
contract StabilizeStrategyETHArbV2 is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public percentSell = 20000; // 20% of the tokens are sold to the cheapest token
    uint256 public maxAmountSell = 200e18; // The maximum amount of tokens that can be sold at once
    ...
    function estimateSellAtMaxSlippage(uint256 originID, uint256 targetID, uint256 _balance) internal view returns (uint256) {
        ...
        _amount = _balance.mul(percentSell).div(DIVISION_FACTOR);
        uint256 _return = simulateExchange(originID, targetID, _amount);
        ...
        for(uint256 i = 0; i < length; i++){
            if(i != targetID){
                ...
                if(_normBal <= minTradeSplit){
                    ...
                    sellBalance = estimateSellAtMaxSlippage(i, localTarget, tokenList[i].token.balanceOf(address(this)));
                }
                uint256 _maxTradeTarget = maxAmountSell.mul(10**tokenList[i].decimals).div(1e18);
    ...
    function startChangeTradingConditions(uint256 _pTradeTrigger, uint256 _pSellPercent,  
                                            ...
                                            uint256 _pMaxStipend, uint256 _maxSell, uint256 _maxSlip) external onlyGovernance {
        // Changes a lot of trading parameters in one call
        require(_pTradeTrigger <= 100000 && _pSellPercent <= 100000 && _pMaxStipend <= 100000 && _maxSlip <= 100000,"Percent cannot be greater than 100%");
        _timelockStart = now;
        _timelockType = 5;
