...
// File: browser/testnft_final.sol
...
contract NEEDLZ is ERC721, Ownable {
    using SafeMath for uint256;
    uint public constant MAX_NEEDLZ = 5;
    bool public hasSaleStarted = false;
    
    // THE IPFS HASH OF ALL TOKEN DATAS WILL BE ADDED HERE WHEN ALL PICKLZ ARE FINALIZED.
    string public METADATA_PROVENANCE_HASH = "1c62a14aa7ca18ace1edf889b668bc22e156d890d9d00f085e63e4f3b7c0d394";
    
    
    constructor() ERC721("Needel NFT Test Token","NEEDLZ")  {
        setBaseURI("https://needel.com/api/");
    ...
    function calculatePrice() public view returns (uint256) {
        ...
        require(totalSupply() < MAX_NEEDLZ, "Sale has already ended");
     ...
     function calculatePriceTest(uint _id) public view returns (uint256) {
        ...
        require(_id < MAX_NEEDLZ, "Sale has already ended");
   ...
   function adoptNEEDLZ(uint256 numNEEDLZ) public payable {
        require(totalSupply() < MAX_NEEDLZ, "Sale has already ended");
       
        uint currentSupply = totalSupply();
       
        if (currentSupply > 2) {
        require(numNEEDLZ > 0 && numNEEDLZ <= 2, "In this tier, you can adopt minimum 1, maximum 2 NEEDLZ");
        } else  {
        require(numNEEDLZ > 0 && numNEEDLZ <= 1, "In this tier, you can adopt minimum 1, maximum 1 NEEDLZ");
        }
        require(totalSupply().add(numNEEDLZ) <= MAX_NEEDLZ, "Exceeds MAX_NEEDLZ");
        require(msg.value >= calculatePrice().mul(numNEEDLZ), "Ether value sent is below the price");

        for (uint i = 0; i < numNEEDLZ; i++) {
