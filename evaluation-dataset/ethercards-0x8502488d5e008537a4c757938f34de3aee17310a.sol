...
contract ethercards is ERC721 , Ownable, Pausable{
    ...
    event OG_Ordered(address buyer, uint256 price_paid, uint256 demand, uint256 tokenId);
    event ALPHA_Ordered(address buyer, uint256 price_paid, uint256 demand, uint256 tokenId);
    event COMMON_Ordered(address buyer, uint256 price_paid, uint256 demand, uint256 tokenId);
...
// need events
   ...
   function setController(address _controller) external onlyOwner {
    ...
    function assignCard(address buyer, uint256 card_type) internal {
        ...
        if (card_type == 2) {
            ...
            emit COMMON_Ordered(msg.sender, msg.value,cStart + cSold+cPending,tokenId);
            uint cSum = cStart+cSold+cPending;
            serialToTokenId[cSum] = tokenId;
            cPending++;
            ...
            emit ALPHA_Ordered(msg.sender, msg.value,aSum,tokenId);
            serialToTokenId[aSum] = tokenId;
            aPending++;
        ...
        emit OG_Ordered(msg.sender, msg.value,oSum,tokenId);
        serialToTokenId[oSum] = tokenId;
        oPending++;
   ...
   function resolve(uint256 random) internal {
        ...
        emit Resolution(card_pos,tokenId,chance);
    }
    ...
    function verifyTokenAt(uint256 position, uint[10000] calldata tokenIdArray) public view returns (bool) {
        ...
        return (tokenIdToSerial[tokenId] > tokenIdToSerial[prevToken] );
    }
    ...
    function cardSerialNumber(uint256 tokenId) public view returns (uint256) {
    ...
    function serialToTokenID(uint256 serial) external view returns (uint256) {
    ...
    function cardType(uint256 tokenId) public view returns(CardType) {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (! _FuzeBlown) {
            return string(abi.encodePacked("https://temp.ether.cards/metadata/",uint256(ct).toString()));
        }
        // reformat to directory structure as below
        string memory d100 = (tokenId / 100).toString();
        string memory m100 = (tokenId % 100).toString(); 
        string memory slash = "/";
        return string(abi.encodePacked(baseURI(),d100,slash,m100));
    }
