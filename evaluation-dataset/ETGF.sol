...
contract ETGF is UtilETGF, WhitelistAdminRole {

	using SafeMath for *;
	uint ethWei = 1 ether;
	address payable private devAddr = address(0x4D8F84D8b5A431214698AC4320f3bb1BAF2555ce);
	address payable private comfortAddr = address(0xCC63C3C6A3C53c2Fa8f4Eb57a4D1eD5e8Ea8Db09);
	...
	function getUserByAddress(address addr, uint roundId) public view returns (uint[14] memory info, string memory inviteCode, string memory beCode) {
		...
		info[10] = user.level;
		uint grantAmount = 0;
		if (user.id > 0 && user.freezeAmount >= 1 ether && user.freezeAmount <= bonuslimit && user.investTimes < 7 && userGlobal.status != 1) {
