...
contract Unifarmv4 is Admin {
    ...
    function claimRewards(address userAddress, uint256 stakeId, uint256 stakedAmount, uint256 totalStake) internal {
     ...
     function _rewardCalculation(
        ...
        if (stakingDetails[userAddress].referrer[stakeId] != address(0)) {
            ...
            require(IERC20(stakingDetails[userAddress].tokenAddress[stakeId]).transfer(
    ...
    function unStake(address userAddress, uint256 stakeId) external whenNotPaused returns (bool) {
        ...
        require(msg.sender == userAddress || msg.sender == _owner, "UNSTAKE: Invalid User Entry");
        ...
        // optional lock check
        if(tokenDetails[stakedToken].optionableStatus)
            require(stakingDetails[userAddress].startTime[stakeId].add(stakeDuration) <= block.timestamp, 
    ...
    function emergencyUnstake(uint256 stakeId, address userAddress, address[] memory rewardtokens, uint256[] memory amount) external onlyOwner {
        ...
        uint stakeAmount = stakingDetails[userAddress].stakedAmount[stakeId];
        stakingDetails[userAddress].isActive[stakeId] = false;
        stakingDetails[userAddress].stakedAmount[stakeId] =0;
        totalStaking[stakingDetails[userAddress].tokenAddress[stakeId]] = totalStaking[stakingDetails[userAddress].tokenAddress[stakeId]].sub(stakeAmount);
        
        IERC20(stakingDetails[userAddress].tokenAddress[stakeId]).transfer(userAddress, stakeAmount);
        
        for(uint i; i < rewardtokens.length; i++)   {
            require(IERC20(rewardtokens[i]).balanceOf(address(this)) >= amount[i],
                "EMERGENCY : Insufficient Reward Balance");
                uint rewardsEarned = amount[i];

                 if (stakingDetails[userAddress].referrer[stakeId] != address(0)) {
                        uint256 refEarned = (rewardsEarned.mul(refPercentage)).div(100 ether);
                        rewardsEarned = rewardsEarned.sub(refEarned);

                        require(IERC20(rewardtokens[i]).transfer(stakingDetails[userAddress].referrer[stakeId], refEarned), "EMERGENCY : Transfer Failed");
