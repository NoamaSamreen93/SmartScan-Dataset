...
// File: contracts/drops/staking/StakingErc721.sol
...
contract StakingErc721 is IERC721Receiver, ReentrancyGuard, Pausable, Ownable {
  using SafeMath for uint256;
  ...
  mapping(address => EnumerableSet.UintSet) private nftTokens;
  ...
  function nftTokensOf(address account) external view returns (uint256[] memory) {
    uint256[] memory tokenIds = new uint256[](nftTokens[account].length());
    for(uint256 i = 0; i < tokenIds.length; i++) {
      tokenIds[i] = nftTokens[account].at(i);
    }
    return tokenIds;
  }
  ...
  function balanceOf(address account) external view returns (uint256) {
    return nftTokens[account].length();
  }
  ...
  function changeAddresses(address _TOKEN, address _NFT) public onlyOwner {
    TOKEN = IERC20(_TOKEN);
    NFT = IERC721(_NFT);
  }
  ...
  function stake(uint256[] calldata tokenIds) external nonReentrant whenNotPaused updateReward(msg.sender) {
    for(uint8 i = 0; i < tokenIds.length; i++) {
      stakeInternal(msg.sender, tokenIds[i]);
    }
  }

  function stakeInternal(address sender, uint256 tokenId) internal {
    require(nftTokens[sender].length().add(1) <= maxAmount, "NFT max amount exceeded");

    NFT.safeTransferFrom(sender, address(this), tokenId);

    totalSupply = totalSupply.add(1);
    nftTokens[sender].add(tokenId);

    emit Staked(sender, tokenId);
  }

  function withdraw() public updateReward(msg.sender) {
    uint256[] memory tokenIds = new uint256[](nftTokens[msg.sender].length());
    for(uint256 i = 0; i < tokenIds.length; i++) {
      tokenIds[i] = nftTokens[msg.sender].at(i);
    }
  ...
  function withdrawNFTInternal(uint256 tokenId) internal {
    require(nftTokens[msg.sender].contains(tokenId), "NFT token not found");
    
    totalSupply = totalSupply.sub(1);
    nftTokens[msg.sender].remove(tokenId);
  ...
  function refill(uint256 reward, uint256 duration)
    external
    onlyOwner
  ...
  function recoverERC20(address tokenAddress, uint256 tokenAmount)
    ...
    onlyOwner
  {
    require(tokenAddress != address(TOKEN), "Cannot withdraw the staking or rewards tokens");
