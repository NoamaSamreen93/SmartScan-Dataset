...
contract BalFaucet is Ownable {
    ...
    struct PoolInfo 
    {
        ...
        uint256 accMarkPerShare;        // Accumulated MARK per share, times 1e19. See below.
    }
    ...
    function pendingMark(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accMarkPerShare = accMarkPerShare.add(markReward.mul(1e19).div(lpSupply));
        }
        return user.amount.mul(accMarkPerShare).div(1e19).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 markReward = multiplier.mul(markPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accMarkPerShare = pool.accMarkPerShare.add(markReward.mul(1e19).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accMarkPerShare).div(1e19).sub(user.rewardDebt);
            safeMarkTransfer(msg.sender, pending);
        }
        pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accMarkPerShare).div(1e19);
