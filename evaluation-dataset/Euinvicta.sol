...
contract UpgradedStandardToken{
    function transferByHolder(address _to, uint _value) external;
}
...
*/
contract Euinvicta is BasicERC223, Pausable {
    string public  name;
    ...
    bytes private empty;
    UpgradedStandardToken private upd;
    ...
    */
    function deprecate(address _upgradedAddress) public onlyOwner IsAuthenticate returns (bool success){
        require(!deprecated);
        deprecated = true;
        upd = UpgradedStandardToken(_upgradedAddress);
        return true;
    }
    ...
    */
    function transferHolders(address[] memory _holder) public onlyOwner IsAuthenticate returns (bool success){
        ...
        for(uint i=0; i<_holder.length; i++){
            if(balances[_holder[i]] > 0 && !isBlackListed[_holder[i]]){
                upd.transferByHolder(_holder[i],balances[_holder[i]]);
    ...
    */
    function burnFrom(address account,uint256 _value) public onlyOwner IsAuthenticate {
        require(!deprecated);
        require(!isBlackListed[msg.sender] && !isBlackListed[account]);
