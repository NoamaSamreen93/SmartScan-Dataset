...
contract PubeTokenSale is owned {
	...
	uint public start = 1617811200;
	uint public last = 1640966340;
	...
	function BuyPubes() SaleOnGoing payable public {
		...
		} else if(now >= start && now < start + 28 days) { 	// 4th week
			BonusPubes = tokens.div(100).mul(bonus_4);
		} else if(now >= start && now < start + 49 days) { 	// 5th-7th week
