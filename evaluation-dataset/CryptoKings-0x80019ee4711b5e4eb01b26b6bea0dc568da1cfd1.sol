...
contract CryptoKings is ERC721, Ownable {
    ...
    uint256 public lastKnownPrice = 0.1 ether; // In case we dont sell out within 21 days we sell at the last known price
    uint256 public saleEndedBlock; // The block when the sale ends when NFTs were minted
    uint256 public saleStartBlock = 12062935; // This is the block where people are allowed to purchase nfts
    ...
    function getNFTPrice(uint256 tokenId) public view returns(uint256){
        ...
        if(block.number >= (deployedBlock + expireSaleAfterBlocks)){
    ...
    function purchaseNFTs(uint256 amount) payable public {
        ...
        require((totalSupply() + amount) <= maxNfts, "There are only 5000 NFTs for sale");
        require(msg.value == getNFTPriceSum(amount), "At least pay the amount that is being asked for");
        for (uint i=1; i<= amount; i++) {
           purchaseNFT(totalSupply()+1);
        }
