...
// File: contracts/gaia/gaiaFarm.sol
...
contract gaiaFarm is Ownable {
    ...
    mapping (address => bool) private _addedLP;

    uint256 public gaiaPerBlock;
    ...
    constructor (
        IERC20 _gaia,
        uint256 _gaiaPerBlock
    )  
        public
    {
        gaia = _gaia;
        gaiaPerBlock = _gaiaPerBlock;
        totalAllocPoint = 0;
    }
    ...
    function getAllocPerShare(uint256 _poolID) external view returns (uint256) {
        return poolInfo[_poolID].accGaiaPerShare;
    }

    function gaiaReward(uint256 _poolID) external view returns (uint256) {
        return (gaiaPerBlock * 10 ** 9).mul(poolInfo[_poolID].allocPoint).div(totalAllocPoint);
    }
    ...
    function pendingGaia(uint256 _poolID, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 gaiaRewardAmount = blocks.mul(gaiaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accGaiaPerShare = accGaiaPerShare.add(gaiaRewardAmount.mul(10 ** 18).div(lpSupply)); // 10 ** 18 to match the LP supply
        }
        return user.amount.mul(accGaiaPerShare).div(10 ** 9).sub(user.rewardDebt);
    }

    // mutative functions
    function setGaiaPerBlock(uint256 _newGaiaPerBlock) external onlyOwner {
        gaiaPerBlock = _newGaiaPerBlock;
    }

    function setEndBlock(uint256 _poolID, uint256 _newEndBlock) external onlyOwner {
        poolInfo[_poolID].endBlock = _newEndBlock;
    }
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate, uint256 _startBlock, uint256 _endBlock)
        ...
        onlyOwner
    {
        require(!_addedLP[address(_lpToken)], 'lp exists already');
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function updatePool(uint256 _poolID) public {
        ...
        if (pool.lastRewardBlock <= pool.endBlock) {
            uint256 blocks = block.number <= pool.endBlock ? block.number.sub(pool.lastRewardBlock) : pool.endBlock.sub(pool.lastRewardBlock);
            
            uint256 gaiaRewardAmount = blocks.mul(gaiaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            pool.accGaiaPerShare = pool.accGaiaPerShare.add(gaiaRewardAmount.mul(10 ** 18).div(lpSupply));
            pool.lastRewardBlock = block.number;
        }
    }
    ...
    function deposit(uint256 _poolID, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accGaiaPerShare).div(10 ** 9).sub(user.rewardDebt);
        ...
        user.rewardDebt = user.amount.mul(pool.accGaiaPerShare).div(10 ** 9);
    ...
    function safeGaiaTransfer(address _to, uint256 _amount) internal {
        uint256 gaiaBal = gaia.balanceOf(address(this));
        if (_amount > gaiaBal) {
            gaia.transfer(_to, gaiaBal);
        } else {
            gaia.transfer(_to, _amount);
        }
    }
}