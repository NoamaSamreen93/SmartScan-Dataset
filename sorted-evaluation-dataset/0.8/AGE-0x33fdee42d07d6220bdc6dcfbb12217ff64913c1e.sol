...
// Dependency file: contracts/GuardVault.sol

// pragma solidity ^0.8.10;

interface GuardVault {
    function initGuardianVault(uint tokenId, uint value) external payable;
}

// Root file: contracts/AGE.sol
...
// import "contracts/GuardVault.sol";

contract AGE is ERC721, ERC721Enumerable, Pausable, Ownable {
    ...
    */
    function mintAge(uint numberOfTokens) public payable whenNotPaused {
        ...
        require(vaultValue >= 0.01 ether, "Ether value for vault is not enough.");

        for (uint i = 0; i < numberOfTokens; i++) {
            _safeMint(msg.sender, _tokenIdCounter.current());
            guardVault.initGuardianVault{value : vaultValue}(_tokenIdCounter.current(), vaultValue);
            mintRecords[_msgSender()][0].push(_tokenIdCounter.current());
    ...
    function preMintAge(
        ...
        uint mintedNum = mintRecords[_msgSender()][preStage].length;
        ...
        uint vaultValue = 0.01 ether;

        for (uint i = 0; i < numberOfTokens; i++) {
            _safeMint(_msgSender(), _tokenIdCounter.current());
            guardVault.initGuardianVault{value : vaultValue}(_tokenIdCounter.current(), vaultValue);
            mintRecords[_msgSender()][preStage].push(_tokenIdCounter.current());
            _tokenIdCounter.increment();
        }
    }
    ...
    */
    function reserveAge(uint numberOfTokens, address to) public onlyOwner payable {
        ...
        uint vaultsValue = msg.value / numberOfTokens;
        for (uint i = 0; i < numberOfTokens; i++) {
            _safeMint(to, _tokenIdCounter.current());
            guardVault.initGuardianVault{value : vaultsValue}(_tokenIdCounter.current(), vaultsValue);
            _tokenIdCounter.increment();
        }
    }
    ...
    function getMintRecord(address minter, uint stage) public view returns (uint[] memory) {
    ...
    function setVault(address vaultAddress) public onlyOwner {
        guardVault = GuardVault(vaultAddress);
    }
