...
contract syac is ERC721, Ownable, Pausable {
    ...
    constructor() ERC721("Spoiled Young Ape Club", "SYAC") { 
     ...
     modifier onlyLegandry() {
       require(legandaryAddress[msg.sender],"Only Legendary Address can call!");
        _;
    }
    ...
    function getTokenId(uint256 _value)public pure returns(string memory){
     ...
     if(_value<10){
        tokenId= string(abi.encodePacked("000", _value.toString()));
     }else if(_value<100){
       tokenId=  string(abi.encodePacked("00", _value.toString()));
     }else if(_value<1000){
       tokenId=  string(abi.encodePacked("0", _value.toString()));
     }else{
     tokenId=uint2str(_value);
    }
    return (tokenId);
    }

    function mintOwner() public onlyOwner {
        require(totalLegendaryMinted<LEGENDARY_SUPPLY,"Legendary Total Minted Reached");
        for(uint256 i=1; i <= LEGENDARY_SUPPLY ; i++){
        _safeMint(msg.sender, (totalSupply+i));
        
        }
        totalSupply +=  LEGENDARY_SUPPLY;
        totalLegendaryMinted +=  LEGENDARY_SUPPLY;
    }
    ...
    function mint(uint256 _count) public payable {
        ...
        require(totalSupply + _count <= MAX_SUPPLY, "MAX_SUPPLY_REACHED");
        require(msg.value >=(_count * MINT_PRICE), "INVALID_ETH_SENT");
        require(totalPublicMinted[msg.sender]+_count<=8,"Maximum NFT Limit reached");
   ...
   function addWhitelist(address[] memory _addresses) external onlyOwner {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(prefixURI, getTokenId(tokenId)));
    }
    ...
    function setSaleStarted(bool _hasStarted) external onlyOwner {
        ...
        function setLegendary(address[] memory _legandaryAddress) external onlyOwner {
            ...
            for(uint8 i=0;i<_legandaryAddress.length;i++){
              legandaryAddress[_legandaryAddress[i]]=true;
    }
        }
