...
// File: contracts/GhostClub.sol

pragma solidity >=0.8.0 <0.9.0;

contract GhostClub is ERC721, Ownable {
    using Strings for uint256;
    ...
    uint256 public constant MAX_SUPPLY = 8550;
    uint256 public constant MAX_MINT_AMOUNT = 15;
    uint256 public constant WHITELIST_MAX_MINT_AMOUNT = 3;
    uint256 public cost = 0.055 ether;
    uint256 public marketingSupply = 150;

    Counters.Counter private _tokenSupply;

    event NewSkullMinted(address sender, uint256 tokenId);
        ...
        if (isOnlyWhitelisted == true) {
            ...
            require(
                _mintAmount <= WHITELIST_MAX_MINT_AMOUNT &&
                    ownerMintedCount + _mintAmount <= WHITELIST_MAX_MINT_AMOUNT,
                _mintAmount > WHITELIST_MAX_MINT_AMOUNT
    ...
    function _processMint(uint256 _mintAmount) internal {
        for (uint256 i = 1; i <= _mintAmount; i++) {
            ...
            emit NewSkullMinted(msg.sender, id);
            _tokenSupply.increment();
        }
    }
    ...
    {
        require(isOnlyWhitelisted == false, "Whitelist only");
    ...
    function airdrop(address[] calldata _users) public payable onlyOwner {
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            ...
            emit NewSkullMinted(_users[i], id);
            _tokenSupply.increment();
            marketingSupply--;
        }
    }
    ...
    function withdraw() public payable onlyOwner {
        ...
        //Team
        (bool successA, ) = payable(owner()).call{value: (balance / 100) * 75}(
            ""
        );
        //Dev
        (bool successB, ) = payable(0x51D8f89Fae2e82c4bAcF6Ec270f36624bc9C6D1E)
            .call{value: (balance / 100) * 15}("");
        //Community mgr
        (bool successC, ) = payable(0x608328a456D3205fFBAcD2E00AaFE2eE2471dd17)
            .call{value: (balance / 100) * 10}("");
        require(successA && successB && successC, "Withdraw failed");
