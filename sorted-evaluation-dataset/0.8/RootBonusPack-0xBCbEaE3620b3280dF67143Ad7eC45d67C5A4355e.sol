...
import {IBonusPackMinter} from "../IBonusPackMinter.sol";
import {ERC165Storage} from "@openzeppelin/contracts/utils/introspection/ERC165Storage.sol";
import {Pausable } from "@openzeppelin/contracts/security/Pausable.sol";
import "@openzeppelin/contracts/interfaces/IERC2981.sol";

contract RootBonusPack is
    AccessControl,
    ...
    Pausable
{
    ...
    IBonusPackMinter public bonusPackMinter;
    ...
    event BonusPackPaused(address owner);
    event BonusPackUnpaused(address owner);
    ...
    function setBonusPackMinter(address _minterAddress) external whenNotPaused onlyOwner {
        bonusPackMinter = IBonusPackMinter(_minterAddress);
    ...
    whenNotPaused
    {
        ...
        if (from != auctionAddress && from != address(0)) {
            require(block.timestamp > (startDate + 14 days), "RootBonusPack: Locked until January 25th");
        }
    }
    ...
    {
        super._pause();
        emit BonusPackPaused(msgSender());
    }
    ...
    {
        super._unpause();
        emit BonusPackUnpaused(msgSender());
    }
}