...
contract Champ is ERC20, Ownable {
    ...
    event StakingAddressUpdated(address indexed newWallet, address indexed oldWallet);
    ...
    constructor() ERC20("Champ", "CHMP") {
        ...
        swapTokensAtAmount = totalSupply * 25 / 100000; // 0.025% swap limit
        ...
        sellMarketingFee = 60;
        sellLiquidityFee = 190;
        sellStakingFee = 0;
        sellTotalFees = sellMarketingFee + sellLiquidityFee + sellStakingFee;
        
    	marketingWallet = address(0x84d6f04A05Ce855Bb9e475d0d10e3bb46a9645C8); // set as marketing wallet
        stakingAddress = address(newOwner);
    ...
    function updateSellFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _stakingFee) external onlyOwner {
    ...
    function updateStakingAddress(address newAddress) external onlyOwner {
        emit StakingAddressUpdated(newAddress, stakingAddress);
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            if (automatedMarketMakerPairs[to] && sellTotalFees > 0){
                ...
                stakingTokens = fees * sellStakingFee / sellTotalFees;
            }
            // on buy
                ...
                stakingTokens = fees * buyStakingFee / buyTotalFees;
            }
            
            if(stakingTokens > 0){
                super._transfer(from, address(stakingAddress), stakingTokens);
            }

            if(fees - stakingTokens > 0){    
                super._transfer(from, address(this), fees-stakingTokens);
            }
