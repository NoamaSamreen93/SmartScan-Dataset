...
abstract contract ArqSkullsPhysical is ERC721Enumerable, Ownable {
    
    
    address private _controlCenter;
    address private _burnAddress;
    uint256 public constant max_skulls = 50;
    uint256 private _maxRedemption = 3; 
    uint256 private _price = 500000000000000000; //0.5 ETH
    uint256 private _discountPerToken = 100000000000000000;
    uint256 private _memberPrice = 300000000000000000;
    uint256 private _saleTime = 1636587413;

    string private _baseTokenURI;
    
    mapping(address => uint256) private _walletMinted;
    mapping(address => string) private _contactShip;
     
    constructor(string memory baseURI)
        ERC721("Arq Box", "SKULL")
    {
        setBaseURI(baseURI);
    ...
    function redeem(string memory _contact, uint256 _token) public {
        ...
        _contactShip[msg.sender] = _contact;
        safeTransferFrom(msg.sender, _burnAddress, _token);
    ...
    function getCallerPrice(uint256 _redeemTokens, uint256 _quantityMint) public view returns (uint256){
        ControlCenter controlCenter = ControlCenter(_controlCenter);
        uint256 _eligibleBalance = controlCenter.getEligibleBalance(address(this),msg.sender);
        if(_redeemTokens > 0){
          require(_eligibleBalance >= _redeemTokens,"Caller ain't got that many tokens to redeem.");
          return SafeMath.sub(SafeMath.mul(_price,_quantityMint), SafeMath.mul(_redeemTokens, _discountPerToken));
        } else {
    ...
    function mint(uint256 _count, uint256 _redeemTokens) public payable {
        ...
        require(msg.value == getCallerPrice(_redeemTokens,_count),"Price was not correct. Please send with the right amount of ETH.");
...
contract ControlCenter {
    ...
    string private _controlCenterVersion;
    constructor(){
        owner = msg.sender;
        _controlCenterVersion = "CC_V1.0.6_REL0123221257";
    }
    
    function getControlCenterVersion() public view returns (string memory){
        return _controlCenterVersion;
    }
    /*
