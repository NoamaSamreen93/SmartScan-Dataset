...
abstract contract ERC721B is Context, ERC165, IERC721, IERC721Metadata {
    ...
    mapping(address => mapping(uint => uint)) internal _ownerHoldingTime;
    ...
    function ownerHoldingTime(address owner, uint tokenId) public view returns (uint) {
        require(_ownerHoldingTime[owner][tokenId] != 0, "ERC721: address not holding this token");
        uint holdingTime = block.timestamp - _ownerHoldingTime[owner][tokenId];
        return holdingTime;
    }
    ...
    function _transfer(
        ...
        _approve(address(0), tokenId);
        _owners[tokenId] = to;
        _ownerHoldingTime[to][tokenId] = block.timestamp;
        _ownerHoldingTime[from][tokenId] = 0;
        
        emit Transfer(from, to, tokenId);
    }
}
// File: contracts/Blimpie/ER721EnumerableLite.sol
...
abstract contract ERC721EnumerableLite is
    ERC721B,
    IERC721Batch,
    IERC721Enumerable
{
...
contract AncientGodsClub is ERC721EnumerableLite, Signed {
    using Strings for uint256;

    uint256 public MAX_DESCENT = 7;
    uint256 public MAX_SUPPLY = 7777;
    uint256 public MAX_GODS_WALLET = 17;
    uint256 public MAX_GODS_PRESALE = 7;
    ...
    uint256 private _saleSupply = 7777;
    string private _tokenURIPrefix = "";
    string private _tokenURISuffix = "";
    string private _tokenURINotRevealedPrefix = "";

    constructor(string memory _preRevealURI, string memory _basePrefix, string memory _baseSuffix)
        Delegated()
        ERC721B("Ancient Gods Club", "AGC", 1)
    {
        setPreRevealURI(_preRevealURI);
        setBaseURI(_basePrefix, _baseSuffix);
    }
        ...
        returns (string memory)
    {
        ...
        if (!reveal) {
            return string(abi.encodePacked(_tokenURINotRevealedPrefix));
        }
    ...
    //external
    function godsDescent(uint256 quantity, bytes calldata signature)
        external
        payable
    {
        require(selling, "Mint not yet started");
        require(quantity <= MAX_DESCENT, "Too many gods");
        require(
            _balances[msg.sender] + quantity <= MAX_GODS_WALLET,
            string(
                abi.encodePacked(
                    "Max ",
                    MAX_GODS_WALLET.toString(),
                    " gods per wallet"
                )
            )
        );
        require(msg.value >= PRICE * quantity, "Ether sent incorrect");

        uint256 supply = totalSupply();
        require(supply + quantity <= _saleSupply, "Mint/order exceeds supply");

        if (presale) {
            require(
                _balances[msg.sender] + quantity <= MAX_GODS_PRESALE,
                string(
                    abi.encodePacked(
                        "Max ",
                        MAX_GODS_PRESALE.toString(),
                        " gods per wallet while presale "
                    )
                )
            );
    ...
    //delegated
    function burnFrom(address account, uint256[] calldata tokenIds)
        external
        payable
        onlyDelegates
    {
    ...
    function resurrect(
        ...
        require(
            ...
            "Must provide equal tokenIds and recipients"
        );
        for (uint256 i; i < tokenIds.length; ++i) {
    ...
    function setActivity(
        ...
        require(
            selling != selling_ || presale != presale_ || reveal != reveal_,
    ...
    function setBaseURI(string memory newPrefix, string memory newSuffix)
        public
        onlyDelegates
    {
    ...
    function setPreRevealURI(string memory newPrefix) public onlyDelegates {
        _tokenURINotRevealedPrefix = newPrefix;
    }

    function setMax(
        uint256 maxWorship,
        uint256 maxSupply,
        uint256 maxGodWallet,
        uint256 saleSupply,
        uint256 maxGodPresale
    ) external onlyDelegates {
        require(
            MAX_DESCENT != maxWorship ||
                MAX_SUPPLY != maxSupply ||
                MAX_GODS_WALLET != maxGodWallet ||
                MAX_GODS_PRESALE != maxGodPresale,
            "New value matches old"
        );
        require(
    ...
    function _mint(address to, uint256 tokenId) internal override {
        ...
        _owners.push(to);
        _ownerHoldingTime[to][tokenId + 1] = block.timestamp;
