...
//Arq Skulls Physical Contract//

contract ArqBox_R1 is ERC721Enumerable, Ownable {
    
    
    address private _controlCenter;
    uint256 public constant max_skulls = 50;
    uint256 private _maxRedemption = 5; 
    uint256 private _mintLimit = 5;
    uint256 private _price = 50000000000000000; //0.5 ETH
    uint256 private _discountPerToken = 10000000000000000;
    uint256 private _memberPrice = 2500000000000000;
    uint256 private _publicSaleTime = 1642958100;
    uint256 private _whitelistSaleTime = 1642956900;
    ControlCenter private cc = ControlCenter(_controlCenter);
    string private _uriPrefix;
    string private _baseTokenURI;
    string private _redeemedTokenURI;
    bool private _redemptionOpen;
    mapping(address => uint256) private _walletMinted;
    mapping(uint256 => string) private _contactShip;
    mapping(uint256 => bool) private _tokenRedeemed;
    
    

    constructor(string memory baseURI, string memory redeemedURI, bool redeemable)
        ERC721("ArqBox_R1", "ARBX1")
    {
        setBaseURI(baseURI);
        setRedeemedURI(redeemedURI);
        _uriPrefix = "ipfs://"; //default to ipfs 
        _redemptionOpen = redeemable;
    ...
    function toggleRedeemable() public onlyOwner returns(bool){
        if (_redemptionOpen) {
            _redemptionOpen = false;
        } else {
            _redemptionOpen = true;
        }
        return _redemptionOpen;
    }
    function getRedeemable() public view returns(bool){
        return _redemptionOpen;
    }
    function fetchContact(uint256 token) public view onlyOwner returns (string memory){
        return _contactShip[token];
    }
    ...
    function setRedeemedURI(string memory newURI) public onlyOwner {
        _redeemedTokenURI = newURI;
    }
    ...
    function _baseURI(uint256 tokenId) internal view virtual override returns (string memory) {
        string memory _returnURI;
        if (_tokenRedeemed[tokenId]){
            _returnURI = _redeemedTokenURI;
        } else {
            _returnURI = _baseTokenURI;
        }
        return string(abi.encodePacked(_uriPrefix,_returnURI));
    }
    ...
    function setWhitelistSaleTime(uint256 _time) public onlyOwner {
        _whitelistSaleTime = _time;
    }

    function getSaleTime(address user) public view returns (uint256) {
    ...
    function getTimeUntilSale (address user) public view returns (uint256){
        ...
        uint256 saleTime =  getSaleTime(user);
    ...
    function setDiscountAmmount(uint256 _newWEIPrice) public onlyOwner {
        _discountPerToken = _newWEIPrice;
    }
    ...
    function redeem(string memory _contact, uint256 _token) public {
        require(_redemptionOpen, "Redemptions are not activated");
        require(msg.sender == ownerOf(_token), "Sender is not the owner of the requested token");
        require(_tokenRedeemed[_token] == false, "Token has already been redeemed");
        require(bytes(_contact).length > 0, "Must provide contact information to redeem.");
        

        _contactShip[_token] = _contact;
        _tokenRedeemed[_token] = true;
    ...
    function mint(uint256 _count) public payable {
        ...
        require(block.timestamp >= getSaleTime(msg.sender), "Sale is not yet open.");
        require(totSupply < max_skulls, "All Skulls have been Claimed.");
        require(totSupply + _count <= max_skulls,"There are not enough Skulls available");
        ...
        /*if(_redeemTokens > 0) {
            ControlCenter cc = ControlCenter(_controlCenter);
            cc.redeemEligibleTokens(address(this),_redeemTokens,msg.sender);
        }*/
        
        for(uint i = 0; i < _count; i++) {
            uint mintIndex = totalSupply();
            if (mintIndex < max_skulls) {
