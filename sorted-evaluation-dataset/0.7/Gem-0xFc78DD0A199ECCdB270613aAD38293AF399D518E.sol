...
interface ILuckyDragons {
    function getUserStakeLength(address owner) external view returns (uint256);
}

contract Gem is ERC20, Ownable {
    ILuckyDragons public LuckyDragons;

    uint256 public constant BASE_RATE = 1000000000000000000; //1 Gem
    ...
    constructor(address _luckyDragons) ERC20("Gem", "GEM") {
        LuckyDragons = ILuckyDragons(_luckyDragons);
        START = block.timestamp;
        _mint(msg.sender, 1000000000000000000000000); //initial supply
    }
    ...
    function updateReward(address from, address to) external {
        require(
            msg.sender == address(LuckyDragons),
    ...
    function getPendingReward(address user) internal view returns (uint256) {
        return
            (LuckyDragons.getUserStakeLength(user) *
                BASE_RATE *
                (block.timestamp -
