    ...
    AbstractSlasher
{
    ...
    modifier onlyLiquidityBridge() {
        require(_msgSender() == liquidityBridgeAddress, "PB: not authorized");
        _;
    }
    ...
    function _stakeBMIX(
        ...
        // transfer bmiX from user to staking
    ...
    function _transferProfit(
        ...
        bmiToken.transfer(destiny, totalProfit);
    ...
    function withdrawBMIProfit(uint256 tokenId) public override {
        ...
        _transferProfit(tokenId, true, _msgSender());
    }
    ...
    function withdrawFundsWithProfit(uint256 tokenId) public override {
        _withdrawFundsWithProfit(_msgSender(), tokenId);
    }

    function migrateWitdrawFundsWithProfit(address _sender, uint256 _tokenId)
        external
        override
        onlyLiquidityBridge
        returns (uint256 _bmiRewards)
    {
        (_bmiRewards) = _withdrawFundsWithProfit(_sender, _tokenId);
    }

    function _withdrawFundsWithProfit(address _sender, uint256 tokenId)
        internal
        returns (uint256 _bmiRewards)
    {
        address policyBookAddress = _stakersPool[tokenId].policyBookAddress;
        ...
        require(ownerOf(tokenId) == _sender, "BDS: Not a token owner");

        _bmiRewards = _transferProfit(tokenId, false, liquidityBridgeAddress);

        uint256 stakedFunds = _stakersPool[tokenId].stakedBMIXAmount;

        // transfer bmiX from staking to the user
        IERC20(policyBookAddress).transfer(_sender, stakedFunds);

        emit StakingFundsWithdrawn(tokenId, policyBookAddress, _sender, stakedFunds);

        _burnStake(_sender, tokenId);
            ...
            for (uint256 j = offset; j < to; j++) {
                ...
                if (_stakersPool[nftIndex].policyBookAddress == policyBooksAddresses[i]) {
                    nftsInfo[i][nftsCount[i]] = NFTsInfo(
                        nftIndex,
                        uri(nftIndex),
                        _stakersPool[nftIndex].stakedBMIXAmount,
